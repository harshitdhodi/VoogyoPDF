...
const ItineraryWithTransport = ({ itineraries = [], transports = [], hotels = [], activities = [] }) => {
  ...
  const transportsByDay = itineraries.reduce((acc, itinerary) => {
    const dayNumber = normalizeDayNumber(itinerary.dayNumber);
    acc[dayNumber] = (transports || []).filter(
      (transport) => formatDate(transport.date) === formatDate(itinerary.startDate)
    );
    return acc;
  }, {});

  console.log("Transports by Day:", transportsByDay);

  // Map hotels by day
  const hotelsByDay = itineraries.reduce((acc, itinerary) => {
    const dayNumber = normalizeDayNumber(itinerary.dayNumber);
    acc[dayNumber] = (hotels || []).filter(
      (hotel) => formatDate(hotel.startDate) === formatDate(itinerary.startDate)
    );
    return acc;
  }, {});

  console.log("Hotels by Day:", hotelsByDay);
  ...
  return (
    <div className="space-y-8">
      {itineraries.map((itinerary) => {
        const dayNumber = normalizeDayNumber(itinerary.dayNumber);
        return (
          <div key={itinerary.id} className="border-t ">
            <h2 className="text-2xl bg-gray-200 px-5 py-2 font-bold">
              {`Day ${dayNumber}`}
            </h2>

            {/* Transports for the day */}
            {transportsByDay[dayNumber]?.map((transport) => (
